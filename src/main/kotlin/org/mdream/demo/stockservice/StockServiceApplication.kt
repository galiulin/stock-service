package org.mdream.demo.stockservice
...
class PriceService {
    fun generatePrices(symbol: String): Flux<StockPrice> {
        return Flux.interval(Duration.ofSeconds(1))
                .map { returnNewStockPrice(symbol) }
    }

    private fun returnNewStockPrice(symbol: String):StockPrice{
        val stockPrice = StockPrice(symbol, randomStockPrice(), now())
        println(stockPrice)
        return stockPrice
    }
